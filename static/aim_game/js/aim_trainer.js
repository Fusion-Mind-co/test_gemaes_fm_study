...
function startBgWiggle(){
  ...
  if (shouldBurst) spawnBackgroundSwapEffect();
    ...
    async function submitResults(aborted) {
        ...
        formData.append('score', Math.round(metrics.score));
        formData.append('kills', metrics.kills);
        formData.append('accuracy', accuracy.toFixed(2));
        formData.append('avg_ttk', avgTtk.toFixed(2));
        formData.append('max_combo', metrics.maxCombo);
        formData.append('misses', metrics.misses);
        formData.append('difficulty', state.presetKey);
        ...
        if (!response.ok) {
            ...
            throw new Error(error && error.errors ? JSON.stringify(error.errors) : 'Unknown error');
        }
